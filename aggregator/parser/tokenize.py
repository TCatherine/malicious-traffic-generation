...
class Tokenizer:
    def __init__(self):
        ...
        # predefine tokens
        self.meta_tokens = dict(unk="<unk>", pad="<pad>", bos="<bos>", eos="<eos>", sep="<sep>")
    ...
    def from_corpus(cls, corpus: str,
                    ...
                    learn_bpe_args: Dict = None):

        bpet = cls()
        save_path = learn_bpe_args['dict_path']
        del learn_bpe_args['dict_path']
    ...
    def tokenize(self, text: str) -> List[str]:
        ...
        maj_flag = False
        upp_flag = False
        tokens.append(self.meta_tokens['bos'])
        ...
        for word in words:
            ...
            if word != words[-1]:
                tokens.append(self.meta_tokens['sep'])

        tokens.extend([
            self.meta_tokens['pad'] for _ in range(
                self.learn_bpe_args['fixed_length'] - len(tokens) - 1
        )])
        tokens = tokens[:self.learn_bpe_args['fixed_length'] - 1]
        tokens.append(self.meta_tokens['eos'])
        return tokens
    ...
    def detokenize(self, tokens: List[str]) -> str:
        ...
        spec_symbols = [self.meta_tokens['bos'], self.meta_tokens['eos'], self.meta_tokens['sep'],
                        self.meta_tokens['pad']]
        filtred_tokens = list(filter(lambda token: token not in spec_symbols, tokens))
        tokens = [token for token in filtred_tokens if token not in self.preprocess_tokens.values()]
        text = "".join(tokens)
        return text
    ...
    @classmethod
    def load(cls, path: Path, fixed_length: int):
        bpet = cls()
        itos = pickle.load(path.open('rb'))
        stoi = {s: i for i, s in enumerate(itos)}
        bpet.vocab_itos = itos
        bpet.vocab_stoi = stoi
        bpet.learn_bpe_args['fixed_length'] = fixed_length
        return bpet
